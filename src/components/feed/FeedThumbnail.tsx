...
import { getGlobalMuted, setGlobalMuted } from "./SinglePlayer";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const FeedThumbnail = memo(({ 
  video, 
  index,
  isActive,
  currentUserId, 
  ...
  useEffect(() => {
    const check = () => setIsMuted(getGlobalMuted());
    const interval = setInterval(check, 100); // Simple poll for mute state
  ...
  return (
    ...
    >
      {/* Thumbnail image - only shown when not active (video overlays when active) */}
      {!isActive && (
        <img 
          src={posterSrc} 
          alt="" 
...
FeedThumbnail.displayName = 'FeedThumbnail';
