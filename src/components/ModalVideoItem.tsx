...
export const ModalVideoItem = memo(({
  video, index, isActive, currentUserId,
  isLiked, isSaved, likesCount, savesCount,
  onToggleLike, onToggleSave, onDelete, onClose, onVideoUpdated,
}: ModalVideoItemProps) => {
  ...
  const handleVideoTap = useCallback((e: React.MouseEvent<HTMLVideoElement>) => {
    ...
    if (timeSinceLastTap > 50 && timeSinceLastTap < 300) {
      ...
      singleTapTimeoutRef.current = setTimeout(() => { unmute(); singleTapTimeoutRef.current = null; }, 300);
    }
  }, [unmute, triggerHeartAnimation, isLiked, onToggleLike, video.id]);

  const handleTimeUpdate = useCallback(() => {
    const videoEl = videoRef.current;
    if (videoEl && !isScrubbing) { setProgress(videoEl.currentTime); setDuration(videoEl.duration || 0); }
  }, [isScrubbing]);
  ...
  const seekToPosition = useCallback((clientX: number) => {
    ...
    videoEl.currentTime = (x / rect.width) * duration;
    setProgress(videoEl.currentTime);
  }, [duration]);

  const handleProgressMouseDown = useCallback((e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    setIsScrubbing(true); seekToPosition(e.clientX);
  ...
  const handleProgressTouchStart = useCallback((e: React.TouchEvent) => {
    e.stopPropagation(); setIsScrubbing(true); seekToPosition(e.touches[0].clientX);
  ...
  const handleProfileClick = () => { onClose(); navigate(`/profile/${video.user_id}`); };
  const handleCategoryClick = (tag: string) => { onClose(); window.location.href = `/?category=${encodeURIComponent(tag)}`; };
  ...
  return (
    <div className="relative w-full h-[100dvh] snap-start snap-always bg-black flex items-center justify-center">
      {posterSrc && <img src={posterSrc} alt="" className="absolute inset-0 w-full h-full object-contain bg-black pointer-events-none" style={{ paddingBottom: navOffset }} />}
          ...
          {localVideo.tags && localVideo.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {localVideo.tags.map((tag, i) => (
                <button key={i} onClick={(e) => { e.stopPropagation(); handleCategoryClick(tag); }} className="text-primary text-sm font-semibold hover:underline cursor-pointer">#{tag}</button>
              ))}
            </div>
          )}
        </div>
      ...
      <EditVideoDialog open={isEditOpen} onOpenChange={setIsEditOpen} videoId={video.id}
        ...
        onSaved={(desc, tags) => { setLocalVideo(prev => ({ ...prev, description: desc, tags })); onVideoUpdated?.(video.id, desc, tags); }} />
    </div>
  );
});
